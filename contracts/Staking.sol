...
contract Staking is Ownable, ReentrancyGuard, ERC721 {
    ...
    constructor(address BUSD) ERC721("Staking", "ST") {
        require(BUSD != address(0x0), "Address Cannot be Zero Address");
        _BUSD = IERC20(BUSD);
    ...
    function staking(uint256 tokenAmount, string memory timeString)
        public
        check(timeString)
    {
    ...
    function returnTimingSchedule(string memory timeInString)
        public
        view
        returns (TimePeriod memory)
    {
